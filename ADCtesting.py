...
class ADCTestingWindow(QtWidgets.QMainWindow, adcwindow.Ui_ADC):
	...
	def __init__(self,main):
		...
		self.afocus1_lineEdit.returnPressed.connect(lambda:self.main.adcthread.move_top(self.afocus1_lineEdit.text()))
		self.afocus2_lineEdit.returnPressed.connect(lambda:self.main.adcthread.move_bottom(self.afocus1_lineEdit.text()))

		self.afocus1_btn_sub.pressed.connect(self.main.adcthread.calc_steps_top)
		self.afocus2_btn_sub.pressed.connect(self.main.adcthread.calc_steps_bottom)
...
class ADCThread(QtCore.QThread):
	...
	def calc_steps_top(self): 
		...
		#equation from https://sites.astro.caltech.edu/~mcs/CBI/pointing/ and Egner et al. 2010
		tanpar = (math.sin(had_r))/((math.tan(obslat_r)*math.cos(dec_r))-(math.sin(dec_r)*math.cos(had_r)))	
		...
		#if dec is higher than 41 degrees par angle should start at 180 instead of 0, and if HA is positive par values should be positive
		#if dec is lower par angle should start at 0, and if HA is negative values are negative.
		if dec_r > obslat_r:
		...
		#calculates rotation angle to move both top and bottom prism to account for dispersion at a given zenith angle, temperature, and pressure
		#0.38 - 0.68 microns range of FHiRE
		lam_min = 0.38 #[microns]
		lam_max = 0.68 #[microns]
		
		#equations (from https://refractiveindex.info/) to determine index of refraction for BaK2 and CaF2 prisms 
		#n1= BaK2, n2 = CaF2
		a0 = 2.319725
		a1 = 0.002602493
		a2 = 0.02249807
		...
		n1_min = np.sqrt(a0+(a1*lam_min**2)+(a2*lam_min**(-2))+(a3*lam_min**(-4))+(a4*lam_min**(-6))+(a5*lam_min**(-8)))
		n1_max = np.sqrt(a0+(a1*lam_max**2)+(a2*lam_max**(-2))+(a3*lam_max**(-4))+(a4*lam_max**(-6))+(a5*lam_max**(-8)))
		...
		n2_min = (b0*lam_min**2)/(lam_min**2-b4) + (b1*lam_min**2)/(lam_min**2-b5) + (b2*lam_min**2)/(lam_min**2-b6) + (b3*lam_min**2)/(lam_min**2-b7)
		n2_min = np.sqrt(n2_min + 1)
		n2_max = (b0*lam_max**2)/(lam_max**2-b4) + (b1*lam_max**2)/(lam_max**2-b5) + (b2*lam_max**2)/(lam_max**2-b6) + (b3*lam_max**2)/(lam_max**2-b7)
		n2_max = np.sqrt(n2_max + 1)

		n2 = n2_max - n2_min

		#equations from Filippenko 1982, Brescia et al. 2006, Tendulkar 2014
		#calculating atmospheric refraction for range of wavelengths min to max
		R_min_sealevel = float((64.328+29498.1/(146-lam_min**(-2))+255.4/(41-lam_min**(-2)))*10**-6)
		R_min_scaled = float((R_min_sealevel)*self.telP*(1+(1.049-0.0157*self.teltemp)*self.telP*10**-6)/(720.883*(1+0.003661*self.teltemp)))

		R_max_sealevel = float((64.328+29498.1/(146-lam_max**(-2))+255.4/(41-lam_max**(-2)))*10**-6)
		R_max_scaled = float((R_max_sealevel)*self.telP*(1+(1.049-0.0157*self.teltemp)*self.telP*10**-6)/(720.883*(1+0.003661*self.teltemp)))

		#atmospheric dispersion in radians
		Delta = (self.telP/(760+(2.9*self.teltemp)))*(R_min_scaled-R_max_scaled)*np.tan(self.z)
		print("Dispersion = %s rads" %Delta)

		#compares value of atmospheric diffraction with needed value of correction and calculates rotation angle
		F = 12700.3 # focal length in mm ***change once finalized in Zemax***
		prismangle = math.radians(5.54) #prism angle
		H = math.tan(prismangle) # normalized prism base
		D = 367.79	# distance between prisms and focus mm ***change once finalized in Zemax***
		dnglass1 = n1 - n2

		Phi = np.degrees(np.arccos(np.tan(F*Delta/(D*dnglass1))/H))-90
		print("Rotation angle = %s degrees" %Phi)
		rotstepsfloat = Phi/.315
		self.rotstepsint = int(rotstepsfloat)
		#total steps=parallactic angle + dispersion compensation angle of rotation
	...
	def move_steps_top(self):
		...
		stepper2.set_direction(True)
		#disabling and enabling outputs prevent motors from getting too hot from idle current
		stepper2.disable()

	#----------------Bottom prism-------------------#
	def calc_steps_bottom(self):
		#bottom steps=top steps - 2*dispersion compensation angle of rotation (since already subtracted from top)
	...
	def updatePosition(self):
		...
		print("RA: %s\nDEC: %s\nUTC: %s\nHUM: %s %%\nTEMP: %s C\nHA: %s\nP: %s mmHg\nAirmass: %s" %(self.telra,self.teldec,self.telutc,self.telhum,self.teltemp,self.telhad,self.telP, self.telairm))
